...
// over the cached segment dumps in ml/room-detection/data/vector-segments plus the
// text-label sidecars in ml/room-detection/data/text-labels, and writes one prediction
// JSON per page under .eval/<run>/predictions/. Scoring against the pdf-tsv ground
// truth happens in ml/room-detection/score_predictions.py (spawn it with --score).
//
// Usage:
...
//   --score           run the Python scorer on the predictions dir afterwards
//   --score-args STR  extra args appended to the scorer invocation (e.g. "--split val")
...
const pythonBin = path.resolve(repoRootDir, "ml", "room-detection", ".venv", "bin", "python");
...
function serializeResult(entry, result, detectMs, options) {
  ...
  return {
    ...
    rooms: result.rooms.map((room) => ({
      polygon: Array.from(room.polygon, (value) => roundTo(value, 2)),
  ...
  try {
    ...
    for (const [index, entry] of entries.entries()) {
      ...
      try {
        ...
        if (args.fromPdf) {
          const pdfPath = path.join(pdfTsvDir, entry.folder, `${entry.stem}.pdf`);
...
function runScorer(args) {
  return new Promise((resolve, reject) => {
    ...
    const scorerPath = path.resolve(repoRootDir, "ml", "room-detection", "score_predictions.py");
    const child = spawn(pythonBin, [scorerPath, "--predictions", predictionsDir, ...args.scoreArgs], {
      stdio: "inherit",
      cwd: path.resolve(repoRootDir, "ml", "room-detection")
    });
    child.on("error", reject);
...
async function main() {
  ...
  console.log(JSON.stringify({ event: "eval_config", run: args.run, entries: entries.length, split: args.split, opts: args.opts, png: args.png, fromPdf: args.fromPdf, jobs: args.jobs }));
