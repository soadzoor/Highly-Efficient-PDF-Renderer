...
try {
  ...
  for (let index = 0; index < count; index++) {
    data.endpoints.set([1500, 0, 1500, 0], index * 4);
    data.primitiveMeta.set([1500, 0, 0, 5], index * 4);
    data.primitiveBounds.set([1500, 0, 1500, 0], index * 4);
  ...
  for (const ordered of [false, true]) for (const backend of ["webgl", "webgpu"]) {
    ...
    try {
      ...
      // A real tilt still excludes density LOD; returning to front-facing enables it.
      ...
      assert.equal(lod.runtime.getRenderedSegmentCount(), count);
      console.log(`${backend}/${ordered ? "ordered" : "legacy"}: Auto LOD ${count}→${initialCount}; 60 MapControls pans, zero reselections; tilt/zoom exact`);
