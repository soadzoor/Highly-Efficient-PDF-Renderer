 ...
 * WGSL. Every band within `radius` of the row `y` is visited; `setup` runs once
 * per band before its segments (with `band`, `bandCount` and `bandInfo` in
 * scope), and the caller's edge math runs between the segment loop's braces.
 */
export function vectorFillBandLoopWgsl(options: {
  bands: string; y: string; radius: string; count: string; start: string;
  texture: string; entries: string; edge: string; setup?: string;
}): string {
  return `
