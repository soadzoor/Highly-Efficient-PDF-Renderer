...
uniform sampler2D uPathMetaTexA;`)
  ...
  .replace("vec2 world = mix(metaA.zw - margin, metaB.xy + margin, corner01);", "vec2 world = aMeshPosition;");
...
export const GRADIENT_MESH_WGSL = GRADIENT_FILL_WGSL
  ...
  .replace("let world = mix(metaA.zw - margin, metaB.xy + margin, corner);", "let world = meshPosition;")
